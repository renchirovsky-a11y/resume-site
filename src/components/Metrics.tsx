...
function AnimatedCounter({
  ...
  return (
    <span className="number-3d block text-4xl font-bold text-ivory sm:text-5xl">
      {prefix}
      {formatCount(count)}
      {suffix}
    </span>
...
export default function Metrics() {
  ...
  return (
    <section className="section-band relative overflow-hidden pt-6" ref={ref}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {metricDefs.map((metric, index) => (
            <m.div
              ...
              className="matte-panel kinetic-panel stat-card card-3d-deep p-6 transition-transform duration-200"
            >
              <div className="panel-inner">
                <div className="mb-8 flex items-center justify-between">
                  <div className={`icon-3d-coin ${index % 2 === 1 ? "icon-3d-coin-amber" : ""} grid h-11 w-11 place-items-center rounded-[8px] border border-[color:var(--line)] bg-white/[0.025] text-[color:var(--teal)]`}>
                    <metric.icon size={20} />
                  </div>
